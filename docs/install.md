...
Static hosting of `dist/` remains supported. Google sign-in, shared organization hosting, and server-side user accounts are planned separately. The local `serve` command only binds to loopback and must not be reverse-proxied onto the internet.
